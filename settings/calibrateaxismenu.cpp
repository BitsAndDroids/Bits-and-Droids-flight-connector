...
void CalibrateAxisMenu::saveSettings(){
    for(int i = 0; i < curves.size(); i++){
        ...
        QStringList sliderNames = {QString::number(i) + curves[i] + "Deadzone", QString::number(i) + curves[i] + "MinSensitivity",
                                   QString::number(i) + curves[i] + "PlusSensitivity"};
        for (const auto &sliderName : sliderNames) {
            auto *sliderFound =
                    findChild<QSlider *>(sliderName);
            cout << sliderFound->objectName().toStdString() << endl;
            settingsHandler.storeSubGroup(
                    curves[i] + "Series", "sliders", sliderName,
                    sliderFound
                    ->value());
        }
        ...
        for (int j = 0; j < coords->size(); j++) {
            cout<<i<<endl;
            settingsHandler.storeSubGroup(curves[i] + "Series", "axis", QString::number(j),
                                          coords->at(j).getX());
            settingsHandler.storeSubGroup(curves[i] + "Series", "value", QString::number(j),
                                          coords->at(j).getY());
        }
...
QGridLayout *CalibrateAxisMenu::GenerateCurveLayouts() {
  ...
  for(auto & curve : curves){
      ...
      if (!settingsHandler
      .retrieveSubSetting(curve + "Series", "sliders", curve + "Deadzone")
      ->isNull()) {
  ...
  connect(saveBtn, &QPushButton::clicked, this, &CalibrateAxisMenu::saveSettings);
