...
enum modes {
    INPUT = 0, INPUTVALUE = 1, OUTPUT = 3, AXIS = 9
};
...
void EventWindow::fillRow(int index) {
    ...
    auto prefixCell = new QTableWidgetItem(
            QString::number(tableRows[index].getPrefix()));

    auto eventCell =
            new QTableWidgetItem(QString::fromStdString(tableRows[index].getEvent()));
    auto typeCell =
            new QTableWidgetItem(QString::number(tableRows[index].getType()));
    auto updateEveryCell = new QTableWidgetItem();
    if (tableRows[index].getType() == 3) {
    ...
    auto commentCell =
            new QTableWidgetItem(QString::fromStdString(tableRows[index].getComment()));
    ...
    switch (tableRows[index].getType()) {
        case INPUT:
    ...
    if (tableRows[index].getDatatype() == "i") {
        dataTypeCombobox->setCurrentIndex(0);
    } else if (tableRows[index].getDatatype() == "f") {
        dataTypeCombobox->setCurrentIndex(1);
    } else if (tableRows[index].getDatatype() == "b") {
...
void EventWindow::cellTextChanged(QTableWidgetItem *changedItem) {
    ...
    switch (changedItem->column()) {
        ...
        case 3:
            textToCompare = std::to_string(tableRows[changedItem->row()].getUpdateEvery());
            break;
        case 4:
...
void EventWindow::saveBtnPressed() {
    ...
    if (!rowsChanged.empty() || !tableRowsToDelete.empty()) {
        ...
        for (auto &toRemove : tableRowsToDelete) {
            ...
            detailedText += QString::fromStdString(tableRows[toRemove].getEvent()) + "\n";
        }

        for (auto &changed : rowsChanged) {
            detailedText +=
                    "OLD VALUE: " + QString::number(tableRows[changed].getPrefix());
            detailedText += (" " + QString::fromStdString(tableRows[changed].getEvent()));
            detailedText +=
    ...
    if (ret == QMessageBox::Save) {
        ...
        for (auto &changedRow : rowsChanged) {
            cout << "CHANGED " << endl;
            tableRows[changedRow].setPrefix(eventTable->item(changedRow, 0)->text().toInt());
            tableRows[changedRow].setEvent(eventTable->item(changedRow, 1)->text().toStdString());
            tableRows[changedRow].setComment(eventTable->item(changedRow, 5)->text().toStdString());
            int newType =
            ...
            tableRows[changedRow].setUpdateEvery(eventTable->item(changedRow, 4)->text().toFloat());
            tableRows[changedRow].setType(newType);
            if (tableRows[changedRow].getType() == 3) {
                int newDatatype =
                ...
                tableRows[changedRow].setDatatype(dtNewStr);
            }
        }
...
void EventWindow::writeFile() {
    ...
    for (auto &row : tableRows) {
        ...
        QString typeString = "";
        if (row.getType() == 3) {
            typeString = QString::number(row.getType()) +
                         QString::fromStdString(row.getDatatype());
        } else {
            typeString = QString::number(row.getType());
        }
        ...
        out << formattedEvent << "^" << typeString
            << "#" + QString::fromStdString(Utils::padLeft(std::to_string(row.getPrefix()), 4))
...
bool EventWindow::checkIfRowChanged(int index) {
    std::array<bool, 3> checks = {(tableRows[index].getPrefix() == eventTable->item(index, 0)->text().toInt()),
                                  tableRows[index].getEvent() == eventTable->item(index, 1)->text().toStdString(),
            //      tableRows[index].updateEvery ==
            //        //                eventTable->item(index,
            //        3)->text().toStdString()== 0,
                                  tableRows[index].getComment() == eventTable->item(index, 4)->text().toStdString()};
    ...
    if (tableRows[index].getType() != typeIndex) {
        cout << "HELP" << endl;
        return true;
    }
    return std::any_of(checks.begin(), checks.end(), true);
}
...
void EventWindow::readFile() {
    ...
    while (std::getline(file, row)) {
        ...
        if (row.size() > 25 && row.at(0) != '/') {
            ...
            newRow->setType(stoi(row.substr(modeDelimiter + 1, 1)));
            if (newRow->getType() == 3) {
                newRow->setDatatype(row.substr(modeDelimiter + 2, 1));
            }

            newRow->setUpdateEvery(stof(row.substr(updateEveryDelimiter + 1,
