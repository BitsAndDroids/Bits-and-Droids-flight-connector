...
static const float SIMCONNECT_CAMERA_IGNORE_FIELD =
    FLT_MAX;  // Used to tell the Camera API to NOT modify the value in this
...
static const DWORD SIMCONNECT_CLIENTDATATYPE_INT8 =
    -1;  //  8-bit integer number
...
SIMCONNECT_ENUM_FLAGS SIMCONNECT_PICK_FLAGS{
    ...
    SIMCONNECT_PICK_SCENERY = 0x04,  // pick scenery/ pick result item is
                                     // scenery object (hSceneryObject is valid)
    SIMCONNECT_PICK_ALL =
...
SIMCONNECT_USER_ENUM SIMCONNECT_INPUT_GROUP_ID;  // client-defined input group
                                                 // ID
SIMCONNECT_USER_ENUM
...
SIMCONNECT_USER_ENUM SIMCONNECT_CLIENT_DATA_ID;  // client-defined client data
                                                 // ID
SIMCONNECT_USER_ENUM
...
{
  DWORD dwRequestID;
  SIMCONNECT_STRINGV(szMetar);  // Variable length string whose maximum size is
                                // MAX_METAR_LENGTH
};
...
SIMCONNECT_REFSTRUCT SIMCONNECT_RECV_FACILITIES_LIST : public SIMCONNECT_RECV {
  DWORD dwRequestID;
  DWORD dwArraySize;
  DWORD dwEntryNumber;  // when the array of items is too big for one send,
                        // which send this is (0..dwOutOf-1)
...
SIMCONNECTAPI SimConnect_MapClientEventToSimEvent(
    HANDLE hSimConnect, SIMCONNECT_CLIENT_EVENT_ID EventID,
...
SIMCONNECTAPI SimConnect_MapClientDataNameToID(
    HANDLE hSimConnect, const char* szClientDataName,
...
SIMCONNECTAPI SimConnect_SubscribeToFacilities(
    HANDLE hSimConnect, SIMCONNECT_FACILITY_LIST_TYPE type,
    SIMCONNECT_DATA_REQUEST_ID RequestID);
