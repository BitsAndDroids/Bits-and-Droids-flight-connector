...
void MainWindow::installWasm() {
    bool customPathFound = settingsHandler.retrieveSetting("Settings","communityFolderPathLE");
    QString pathfound;
    ...
    if(customPathFound){
        ...
        pathfound =  settingsHandler.retrieveSetting("Settings","communityFolderPathLE")->toString();
    } else{

        QString windowsStorePath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) +
        ...
        QString steamPath = QStandardPaths::writableLocation(QStandardPaths::RuntimeLocation) +
...
MainWindow::MainWindow(QWidget *parent)
    ...
    // SIGNALS + SLOTS
    connect(WasmUpdateEventFile, &QAction::triggered, this, &MainWindow::localUpdateEventFile);
    connect(WasmUpdateEventFile, &QAction::triggered, this, &MainWindow::localUpdateEventFile);
...
void MainWindow::localUpdateEventFile(){
    if(dualThread.isRunning()){
        connect(this, &MainWindow::updateEventFile, &dualThread, &DualWorker::updateEventFile);

        emit updateEventFile();
    } else if(inputThread.isRunning()){

        connect(this, &MainWindow::updateEventFile, &inputThread, &InputWorker::updateEventFile);
        emit updateEventFile();
    }
}
